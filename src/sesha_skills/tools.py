"""Everyday Sesha tools exposed over MCP.
...
def duckduckgo_search(query: str, limit: int = 5) -> list[dict]:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 Sesha"})
    try:
...
def remind(in_minutes: int, text: str) -> dict:
    ...
    if run(["bash", "-c", "command -v at"]).ok:
        cmd = f"echo \"notify-send Sesha '{safe}'\" | at now + {in_minutes} min"
        r = run(["bash", "-c", cmd])
    else:
        r = run(["bash", "-c",
                 f"( sleep {in_minutes * 60}; notify-send Sesha '{safe}' ) & disown"])
